...
export function contactReducers(state = initialContactState, action: ContactActions.Actions) {
	switch (action.type) {
		case ContactActions.LOAD_CONTACTS_SUCCESS:
			
			console.log('reducer load contact success, count: ' + action.payload['contacts'].length);
...
export const { selectAll, selectEntities, selectIds, selectTotal } = adapter.getSelectors(getContactState);
