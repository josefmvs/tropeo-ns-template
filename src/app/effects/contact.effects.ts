...
import { catchError, map,concat, concatMap, mapTo, mergeMap, merge, tap, switchMap } from 'rxjs/operators';
...
export class ContactEffects {
    ...
    loadContacts : Observable<Action>  = this.actions$
        .pipe(
            ofType(
            ContactActions.LOAD_CONTACTS
            ),
            switchMap((action) => 
                this.contactService.dbGetContacts()
            )
    ...
    @Effect()
    addingContact : Observable<Action>  = this.actions$
        .pipe(
            ofType(
            '[CONTACT] Adding'
            ),
    ...
    deleteContact$  = this.actions$
        .pipe(
            ofType(
            ContactActions.REMOVE_CONTACT
            ),
            switchMap((action) => 
                this.contactService.dbRemoveContact(action['id'])
            )
            ,map((_id) =>  (
                {type: '[CONTACT] Remove Success', payload: _id}
